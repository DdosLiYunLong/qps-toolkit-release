#!/bin/bash
###############################################################################
# QPS Toolkit 一键安装脚本 - Ubuntu 22.04 / 20.04
###############################################################################
# 
# 功能：
...
check_system() {
    ...
    if [[ "$OS" != *"Ubuntu"* ]]; then
        log_warning "此脚本为 Ubuntu 设计，当前系统：$OS"
...
# 安装基础工具
install_basics() {
    log_info "安装基础工具 (curl, wget, git, build-essential)..."
    sudo apt install -y curl wget git build-essential > /dev/null 2>&1
